#!/usr/bin/env bash
#
# ragctl CLI End-to-End Test Suite
...
setup() {
    ...
    echo "ragctl CLI E2E Test Results" > "$RESULTS_FILE"
...
ragctl is a production-ready document processing pipeline.
...
echo "========================================="
echo "  ragctl CLI E2E Test Suite"
echo "========================================="
echo ""
...
run_test "Chunk: Simple file" \
    "ragctl chunk ${TEST_DIR}/fixtures/simple.txt"

run_test "Chunk: Medium file" \
    "ragctl chunk ${TEST_DIR}/fixtures/medium.txt"

# Output formats
run_test "Chunk: Output JSON" \
    "ragctl chunk ${TEST_DIR}/fixtures/simple.txt -o ${TEST_DIR}/output/simple.json"

run_test "Chunk: Output JSONL" \
    "ragctl chunk ${TEST_DIR}/fixtures/simple.txt -o ${TEST_DIR}/output/simple.jsonl"

run_test "Chunk: Output CSV" \
    "ragctl chunk ${TEST_DIR}/fixtures/simple.txt -o ${TEST_DIR}/output/simple.csv"
...
run_test "Chunk: Strategy semantic" \
    "ragctl chunk ${TEST_DIR}/fixtures/medium.txt --strategy semantic -o ${TEST_DIR}/output/semantic.json"

run_test "Chunk: Strategy sentence" \
    "ragctl chunk ${TEST_DIR}/fixtures/medium.txt --strategy sentence -o ${TEST_DIR}/output/sentence.json"

run_test "Chunk: Strategy token" \
    "ragctl chunk ${TEST_DIR}/fixtures/medium.txt --strategy token -o ${TEST_DIR}/output/token.json"

# Parameters
run_test "Chunk: Custom max-tokens" \
    "ragctl chunk ${TEST_DIR}/fixtures/medium.txt --max-tokens 200 -o ${TEST_DIR}/output/tokens_200.json"

run_test "Chunk: Custom overlap" \
    "ragctl chunk ${TEST_DIR}/fixtures/medium.txt --overlap 100 -o ${TEST_DIR}/output/overlap_100.json"

run_test "Chunk: With --show flag" \
    "ragctl chunk ${TEST_DIR}/fixtures/simple.txt --show"
...
run_test "Chunk: Strategy auto (simple)" \
    "ragctl chunk ${TEST_DIR}/fixtures/simple.txt --strategy auto -o ${TEST_DIR}/output/auto_simple.json"

run_test "Chunk: Strategy auto (medium)" \
    "ragctl chunk ${TEST_DIR}/fixtures/medium.txt --strategy auto -o ${TEST_DIR}/output/auto_medium.json"
...
run_test "Batch: Basic processing" \
    "ragctl batch ${TEST_DIR}/batch -o ${TEST_DIR}/output/batch_all.json"

run_test "Batch: Output JSONL" \
    "ragctl batch ${TEST_DIR}/batch -o ${TEST_DIR}/output/batch_all.jsonl"

run_test "Batch: Output CSV" \
    "ragctl batch ${TEST_DIR}/batch -o ${TEST_DIR}/output/batch_all.csv"

run_test "Batch: Recursive processing" \
    "ragctl batch ${TEST_DIR}/batch --recursive -o ${TEST_DIR}/output/batch_recursive.json"

run_test "Batch: Pattern matching" \
    "ragctl batch ${TEST_DIR}/batch --pattern '*.txt' -o ${TEST_DIR}/output/batch_pattern.json"

run_test "Batch: Custom strategy" \
    "ragctl batch ${TEST_DIR}/batch --strategy sentence -o ${TEST_DIR}/output/batch_sentence.json"

run_test "Batch: Custom max-tokens" \
    "ragctl batch ${TEST_DIR}/batch --max-tokens 300 -o ${TEST_DIR}/output/batch_300.json"
...
run_test "Eval: Single file" \
    "ragctl eval ${TEST_DIR}/output/semantic.json"

run_test "Eval: Compare strategies" \
    "ragctl eval ${TEST_DIR}/output/semantic.json ${TEST_DIR}/output/sentence.json ${TEST_DIR}/output/token.json --compare"

run_test "Eval: With report output" \
    "ragctl eval ${TEST_DIR}/output/semantic.json --report ${TEST_DIR}/output/eval_report.json"
...
run_test "Error: Non-existent file" \
    "ragctl chunk /nonexistent/file.txt" \
    2

run_test "Error: Invalid strategy" \
    "ragctl chunk ${TEST_DIR}/fixtures/simple.txt --strategy invalid" \
    1

run_test "Error: Tokens too low" \
    "ragctl chunk ${TEST_DIR}/fixtures/simple.txt --max-tokens 10" \
    2

run_test "Error: Tokens too high" \
    "ragctl chunk ${TEST_DIR}/fixtures/simple.txt --max-tokens 5000" \
    2

run_test "Error: Non-existent directory (batch)" \
    "ragctl batch /nonexistent/dir" \
    2

run_test "Error: Empty directory (batch)" \
    "ragctl batch ${TEST_DIR}/output --pattern '*.nonexistent'"
...
log_info "  ragctl ingest ${TEST_DIR}/output/semantic.json"
log_info "  ragctl search 'test query'"
...
START_TIME=$(date +%s)
ragctl batch ${TEST_DIR}/batch -o ${TEST_DIR}/output/perf_batch.json > /dev/null 2>&1
END_TIME=$(date +%s)
