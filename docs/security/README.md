...
ragctl v0.1.3 includes comprehensive security features to protect against common vulnerabilities.
...
**Example:**
```bash
$ ragctl chunk /path/to/symlink.txt
✗ Symlink blocked: /path/to/symlink.txt
Set RAGCTL_ALLOW_SYMLINKS=true to enable.
```
...
**Configuration:**
```bash
export RAGCTL_MAX_FILE_SIZE_MB=500
```
...
```bash
# Security settings
export RAGCTL_MAX_FILE_SIZE_MB=100
export RAGCTL_MAX_BATCH_FILES=10000
export RAGCTL_ALLOW_SYMLINKS=false
export RAGCTL_SANITIZE_METADATA=true
```
